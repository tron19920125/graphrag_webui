...
def excel_to_txt(file_path, project_name):
    ...
    if file_path.endswith(".xlsx"):
        ...
        with open(
            f"{root_dir}/projects/{project_name}/input/{file_name}.txt", "w", encoding="utf-8"
        ) as f:
...
def prepare_file(file_path, file, project_name):
    if file.endswith(".xlsx") or file.endswith(".csv"):
        if has_download_files(file_path):
            ...
            run_command(
                f"cp -r '{file_path}' {root_dir}/projects/{project_name}/input/")
            logger.info(f"copied {file} to input")
    if file.endswith(".txt"):
        run_command(f"cp -r '{file_path}' {root_dir}/projects/{project_name}/input/")
        logger.info(f"copied {file} to input")

    if file.endswith(".md"):
        run_command(
            f"cp -r '{file_path}' {root_dir}/projects/{project_name}/input/{file}.txt"
        )
...
def download_file(doc_url, index, df_count, project_name):
    ...
    os.makedirs(f"{root_dir}/projects/{project_name}/input", exist_ok=True)
    file_path = os.path.join(f"{root_dir}/projects/{project_name}/input", file_name)
